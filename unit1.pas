...
uses
  ...
  Grids, StrUtils, LConvEncoding, Utils;
...
type
  ...
  TBIN_DB = class(TForm)
    ...
    procedure LoadBinDbClick(Sender: TObject);
  private
...
begin
  ...
  with DataChannel do begin
     ...
     //ShowMessage(IntToStr(length(wDLISName)));
...
procedure TBIN_DB.LoadBinDbClick(Sender: TObject);
var fileLen: longWord;
    RecordLength, GridCounter: longWord;
    RecordType: Char;
    pStr, wStr: String;
  ...
  if OpenDialog1.Execute then begin
     ...
     repeat
        ...
        if RecordType = 'L' then begin
            RecordLength:= RecordLength or (Bytes[Counter] shl 16);
            Counter:= Counter + 1;
            RecordLength:= (RecordLength or (Bytes[Counter] shl 24)) - 1;
            Counter:= Counter + 1;
            RecordType:= Chr(Bytes[Counter]);
            Counter:= Counter + 1;
        end;
        GetData(RecordLength);
        if RecordType = 'P' then pStr:= pStr + DataToStr + Line
        else if RecordType = 'M' then pStr:= pStr + MeasurementToStr(ParseMeasurement)
             else if RecordType = 'D' then begin
                     DataChannel:= ParseDataChannel(RecordLength);
                     insert(DataChannel, DataChannels, ChannelCounter + 1);
                     ChannelCounter:= ChannelCounter + 1;
                     pStr:= pStr + DataChannelToStr(DataChannel);
                  end
                  else if RecordType = 'F' then begin
                           if FirstF then begin
                              DataGrid.ColCount:= ChannelCounter+1;
                              for i:=0 to ChannelCounter - 1 do DataGrid.Cells[i+1, 0]:= DataChannels[i].DLISName;
                              for i:=0 to ChannelCounter - 1 do wStr:= wStr + AddCharR(' ', DataChannels[i].DLISName, 16);
                              wStr:= wStr + Line;
                           end;
                           DataGrid.InsertRowWithValues(GridCounter, ParseFrame(DataChannels));
                           DataGrid.Cells[0, GridCounter]:= IntToStr(RecordLength);
                           GridCounter:= GridCounter + 1;
                           FirstF:= False;
                           wStr:= wStr + FrameToStr(ParseFrame(DataChannels));
                       end
...
procedure TBIN_DB.Button1Click(Sender: TObject);
var a: Single;
    aBytes: array[1..3] of Byte absolute a;
begin
  ShowMessage(IntToStr(FillLongWord($FF, $00, $00, $01)));
end;
